...
export default function BookingCalculator({
  ...
  if (data?.pricing?.type === "perPerson") {
    newBasePrice = (categorys?.data || []).reduce((sum, cat) => {
      const name = cat?.name;
      const count = counts[name] || 0;
      const price = data?.pricing?.perPersonPrices?.[name] || 0;
      return sum + count * price;
    }, 0);
  } else if (data?.pricing?.type === "groupTier") {
    const totalPeople = (categorys?.data || []).reduce(
      (sum, cat) => sum + (counts[cat?.name] || 0),
      0
    );
...
>
  {(() => {
    const selected = (categorys?.data || [])
      .filter(cat => (counts[cat?.name] || 0) > 0)
      .map(cat => {
        const count = counts[cat?.name] || 0;
        // For pluralization: if count > 1, use the name as is; else make it singular by removing trailing 's'
        const baseLabel = cat?.name?.replace(/s$/, "");
        return `${count} ${count > 1 ? cat?.name : baseLabel}`;
      });

    return selected.length > 0 ? selected.join(", ") : "Select People";
  })()}
								...
								<div className="flex items-center gap-2">
									<button
										onClick={() =>
											setCounts((prev) => ({
												...prev,
												[cat?.name]: Math.max(0, (prev[cat?.name] || 0) - 1),
											}))
										}
